    ...
    def after_dataset_search(self,search_results: dict[str, Any], search_params: dict[str, Any]):
        try:
            ...
            for dataset in search_results.get('results', []):
                ...
                log.info(f"[DataJsonAPI] after_dataset_search package['id']= {package_id} consolidado {calificacion}")
    ...
    def get_calificacion_dataset(self,id,context):
        try:
            ...
            if existing:
                ...
                log.error(f"[DataJsonAPI][get_calificacion_dataset] rating con id {data}")

                return data
            
        except Exception as e:
            log.error(f"[DataJsonAPI][get_calificacion_dataset] rating con id {id} error={e}")
            return {
                'resource_id': id,
                'average': 0.0,
                'count': 0,
